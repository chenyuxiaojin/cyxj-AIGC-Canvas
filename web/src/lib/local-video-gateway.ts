...
export function localVideoGatewayUrl(address: string) {
    try {
        ...
        if (url.origin === "https://api.chatgpt-code.com" && !url.username && !url.password && !url.search && !url.hash
            && /^\/v1\/(models|chat\/completions)$/.test(url.pathname)) {
            return `/api/ai/guoguo${url.pathname}`;
        }
