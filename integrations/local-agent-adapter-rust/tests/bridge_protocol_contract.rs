...
use local_agent_adapter::{
    ...
    BridgeServer, SqliteCanvasAdapter, CanvasOperation, CanvasOperationAdapter,
...
impl Fixture {
    fn new() -> Self {
        ...
        drop(connection);
        let canvas =
            Arc::new(SqliteCanvasAdapter::open_with_protocol(&database, Arc::new(FixtureProtocol)).unwrap());
        canvas
    ...
    fixture
        ...
        .apply_operations(
            AgentOperationRequest {
                ...
                base_revision: fixture.canvas.get_project("agent-review-wall").unwrap().revision,
                actor: Actor::Agent,
                operations: vec![CanvasOperation::SetProjectTitle {
...
fn allowlisted_video_ingest_creates_one_shared_video_node_and_task() {
    ...
    assert_eq!(completed_project.project["operationState"]["revision"], 3);
    assert_eq!(
    ...
    assert_eq!(fixture.canvas.get_project("project-1").unwrap().project["operationState"]["revision"], 3);
}
...
fn allowlisted_media_reference_nodes_require_runtime_verification() {
    ...
    let request = AgentOperationRequest {
        ...
        base_revision: fixture.canvas.get_project("project-1").unwrap().revision,
        actor: Actor::Agent,
        operations: vec![
...
fn allowlisted_image_ingest_creates_one_final_image_node_without_playback_urls() {
    ...
    assert_eq!(project.project["operationState"]["revision"], 1);
...
fn paid_video_generation_queues_and_starts_once_without_canvas_approval() {
    ...
    missing_keyframe.base_revision = "2".to_owned();
    ...
    bad_resolution.base_revision = "2".to_owned();
    let invalid = fixture
        .client()
...
fn completed_legacy_ingest_repairs_its_loading_node_without_rewriting_the_task() {
    ...
    assert_eq!(legacy.project["operationState"]["revision"], 3);
    ...
    assert_eq!(repaired.project["operationState"]["revision"], 4);
    assert_eq!(
    ...
    fixture
        ...
        .get("/v1/tasks/media-legacy-ingest")
        .unwrap();
    assert_eq!(fixture.canvas.get_project("project-1").unwrap().project["operationState"]["revision"], 4);
}
