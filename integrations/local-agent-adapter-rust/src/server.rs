    ...
    for attempt in 0..3 {
        ...
        if current_status == Some(status) {
            return Ok(crate::ProtocolOutcome {
                ...
                ok: true,
                duplicate: true,
                previous_revision: project.project.pointer("/operationState/revision").and_then(Value::as_u64).unwrap_or(0),
                revision: project.project.pointer("/operationState/revision").and_then(Value::as_u64).unwrap_or(0),
                error_code: None,
                error_message: None,
                error: None,
            });
