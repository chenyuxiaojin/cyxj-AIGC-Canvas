...
fn canonical_operation(operation: &CanvasOperation) -> Result<Value, BridgeError> {
    Ok(match operation {
        ...
        CanvasOperation::CreateNode { node } => json!({ "type": "node.create", "node": node }),
        CanvasOperation::UpdateNode { node_id, patch } => {
            json!({ "type": "node.update", "nodeId": node_id, "patch": patch })
        }
        CanvasOperation::DeleteNode { node_id } => json!({ "type": "node.delete", "nodeId": node_id }),
        CanvasOperation::UpdateProject { patch } => match patch.get("title").and_then(Value::as_str) {
            Some(title) => json!({ "type": "project.update", "title": title }),
            None => return Err(BridgeError::invalid("协议批次的 update_project 仅支持 title。")),
        },
        CanvasOperation::SetGroupMembers { .. } => {
            return Err(BridgeError::invalid("协议批次不支持 set_group_members。"))
        }
...
fn validate_request(request: &AgentOperationRequest) -> Result<(), BridgeError> {
    ...
    for operation in &request.operations {
        match operation {
            ...
            // 通用操作由 crate::operations 在应用时校验。
            CanvasOperation::CreateNode { .. }
            | CanvasOperation::UpdateNode { .. }
            | CanvasOperation::DeleteNode { .. }
            | CanvasOperation::SetGroupMembers { .. }
            | CanvasOperation::UpdateProject { .. } => {}
            CanvasOperation::RemoveConnection { connection_id } => {
                validate_identifier("connection_id", connection_id, 64)?;
            }
        }
